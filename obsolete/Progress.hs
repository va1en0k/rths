...
parMapM'r cnt f xs = do
  let perThread =
        1 + (floor $ fromIntegral (length xs) / fromIntegral chunkCount)
  ...
  let act xs mv =
        mapM f xs
          >>= (\rs -> rs `deepseq` modifyIORef' cnt (+ 1) >> putMVar mv rs)
...
mapWithProgressBar f xs = do -- let total = length xs
...
mapWithProgressBar' f xs = iter 0 xs
 where
  ...
  -- iter :: Double -> [a] -> IO [b]
  iter _         []       = return []
  iter totalTime (x : xs) = do
    let left     = fromIntegral (length xs)
