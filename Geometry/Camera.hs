...
mkCamera from at vup vfov aspect focusDist =
  Camera {
    cLowerLeftCorner = from <-> (u .^ (halfWidth * focusDist)) <-> (v .^ (halfHeight * focusDist)) <-> (w .^ focusDist),
...
getRay c u v = Ray (cOrigin c) (cLowerLeftCorner c <+> ((cHorizontal c) .^ u) <+> ((cVertical c) .^ v) <-> (cOrigin c))
